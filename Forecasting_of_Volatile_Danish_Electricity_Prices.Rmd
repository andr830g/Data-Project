...
prices["Sun"] <- ifelse(prices$Wday %in% c("Sun", "sø", "Søn"), 1, 0)
prices["Sat"] <- ifelse(prices$Wday %in% c("Sat", "lø", "Lør"), 1, 0)
prices["Fri"] <- ifelse(prices$Wday %in% c("Fri", "fr", "Fre"), 1, 0)
prices["Thu"] <- ifelse(prices$Wday %in% c("Thu", "to", "Tor"), 1, 0)
prices["Wed"] <- ifelse(prices$Wday %in% c("Wed", "on", "Ons"), 1, 0)
prices["Tue"] <- ifelse(prices$Wday %in% c("Tue", "ti", "Tir"), 1, 0)
prices["Mon"] <- ifelse(prices$Wday %in% c("Mon", "ma", "Man"), 1, 0)
...
axis(side=1, 
     ...
     labels=c("2020-01-01", "Lockdown", "2020-10-06", "Ever Given","2021-07-12", "Invasion", "2022-04-17", "CPI peak", "2023-01-21"), 
     las=2, cex.axis=0.85)
...
The third data set contains data on consumption in kWh from different grid companies per hour, in the period 2020-01-01 hour 0 (midnight), to 2023-01-21 hour 23.
...
After reading the data, we made sure, that all our variables were read as the correct datatypes, and transformed all missing values into NA values. In the "Production" data set, we also dropped variables from the data set, that had too many missing values, to be useful for analysis. In the "Consumption" data set, we only had three variables, but still a lot of missing values. To keep as much information from the "Consumption" data set as possible, we transformed all the missing values to 0's.

After having cleaned the columns, we then went on to the rows. We found that the start and end times of our three data sets did not match. All of the data sets had the same first observation regarding time, but the "Consumption" data set, was missing some observations at the end. Hence, we had to cut off data from the other two data sets, to make them all equal length.

We then had a strange problem, where there were three seemingly random observations, that were missing from the "Production" data set. We then had to use interpolation to add the previous observation's values to the missing rows, to fill in the missing observations.
...
At last, we also divided the columns in the "Consumption" data set with 1000, since we found that the electricity used in the "Consumption" data set was in kwh, where we had electricity in Mwh in the other data sets.
...
```{r}
acf(prices$spot_price_dkk, lag.max = 1000, main="Spot price in DK1")
```
...
We observe that the two tests do not agree. The null hypothesis for the ADF test is "Not stationary", while the null hypothesis for the KPSS test is "Stationary".
...
A different and also very common approach is to first difference the data, so that every observation is subtracted from the previous observation.
...
plot(prices$spot_price_dkk, 
     main="Trend function", ylab="DKK per MWh", xlab="Time", 
     col=alpha("black", 0.5), type="l")
lines(prices$t, prices_trend, col="red")

plot(prices$t, residuals_trend, 
     main="Detrended data", ylab="Detrended DKK per MWh", xlab="Time", 
     col=alpha("black", 0.5), type="l")
```
...
```{r}
acf(residuals_trend, lag.max=1000, main="Detrended data")
```
...
plot(subset_prices$t, subset_prices$spot_price_dkk, 
     main="Trend function", ylab="DKK per MWh", xlab="Time", 
     col=alpha("black", 0.5), type="l")
lines(subset_prices$t, prices_trend, col="red")

plot(subset_prices$t, residuals_trend, 
     main="Detrended data", ylab="Detrended DKK per MWh", xlab="Time",
     col=alpha("black", 0.5), type="l")
```
...
We also tried using a nonparametric method to fit the underlying trend in the data, but this is of course not a viable solution, since it is nonparametric, and will not work for forecasting.
...
plot(prices$t, prices$spot_price_dkk, 
     main="Nonparametric trend", ylab="DKK per MWh", xlab="Time", type='l')
lines(prices$t, prices_trend, col='red', lwd=2)

plot(prices$t, prices$spot_price_dkk - prices_trend, 
     main="Detrended data", ylab="Detrended DKK per MWh", xlab="Time", type='l')

plot(diff(prices$spot_price_dkk, 1), main="First Differenced", type="l", ylab="Differenced DKK per MWh", xlab="Time")
```

This made us decide on using first differencing to transform our data, to become covariance stationary.

```{r}
acf(diff(prices$spot_price_dkk, 1), lag.max = 1000, main="First Differenced")
acf(diff(prices$spot_price_dkk[prices$Hour==17], 1), lag.max = 1000, main="First Differenced Hour 17")
```

As we can see, the ACF plots for the first differenced data show a fast convergence towards insignificant autocorrelation, and this becomes even more obvious when subsetting on a single hour, to account for seasonality.
...
In this section and in the section "Results", we are referring to our work done in Modelling.Rmd.
...
First, we needed to generate some naive benchmark models, to have some measures to evaluate our different model's performances up against. We used a simple linear regression, a sinusoidal regression and the best fitted sum of multiple sinusoidal regressions we found from the section "Trends", to create some very naive benchmark models. We trained the models on the data without the last 28 days, and then used the last 28 days as validation data.

The results of these benchmark models is shown in the Results table in the section "Results", where we could observe that the simple linear regression gave the best results.
...
$$
\Delta_d y_t = \sum_{i=1}^{p} \beta_i \Delta_d y_{t-i} + \sum_{j=1}^{q} \gamma_j \Delta_d u_{t-j} + \sum_{k=1}^{P} \zeta_k \Delta_D y_{t-T\cdot k} + \sum_{l=1}^{Q} \eta_l \Delta_D u_{t-T\cdot l} + \Delta_d u_t
$$
...
##. SARIMA(24, 1, 1)(7, 0, 1)(24) + weekend + wind + fossile + biomass + waste + exchange
```{r}
...
The auto.arima()-function came with very different suggestions of models, depending on what initial guess was given to the model. Hence, we gave it the initial guess of the best SARIMA(24,1,1)(7,0,1) model, which produced a suggested ARIMA(23,1,0) model for both with and without exogenous variables.

We then tried to use this model for day-ahead forecasting without exogenous variables and saw that it was worse, with an RMSE of 369.8, compared to the RMSE of 344.0 of the SARIMA(24,1,1)(7,0,1) model. When exogenous variables were included, it was instead on pair, with an RMSE of 262.3 compared to the 263.9 of the SARIMA(24,1,1)(7,0,1) model. This can also be seen in the Results table.
...
test_prices["Sun"] <- ifelse(test_prices$Wday %in% c("Sun", "sø", "Søn"), 1, 0)
test_prices["Sat"] <- ifelse(test_prices$Wday %in% c("Sat", "lø", "Lør"), 1, 0)
test_prices["Fri"] <- ifelse(test_prices$Wday %in% c("Fri", "fr", "Fre"), 1, 0)
test_prices["Thu"] <- ifelse(test_prices$Wday %in% c("Thu", "to", "Tor"), 1, 0)
test_prices["Wed"] <- ifelse(test_prices$Wday %in% c("Wed", "on", "Ons"), 1, 0)
test_prices["Tue"] <- ifelse(test_prices$Wday %in% c("Tue", "ti", "Tir"), 1, 0)
test_prices["Mon"] <- ifelse(test_prices$Wday %in% c("Mon", "ma", "Man"), 1, 0)
...
###. SARIMA(24, 1, 1)(7, 0, 1)(24) + weekend + wind + fossile + biomass + waste + exchange on 28 days of test data
...
###. SARIMA(24, 1, 1)(7, 0, 1)(24) + weekend + wind + fossile + biomass + waste + exchange on 59 days of test data
...
We observe that the SARIMA(24,1,1)(7,0,1) model performs exceptionally well on the new test data with an RMSE of 175.7 on the first 28 days of the test data and an RMSE of 153.4 on the 59 days of the test data.

#. Conclusion

In conclusion, we have in this project managed to find a model, based on analysis of the trends, stationarity and seasonality of the time series, which outperforms naive benchmark models, such as the linear regression and simple autoregressive models.

The model we found to be the most accurate, was the SARIMA(24,1,1)(7,0,1)(24) model. This model's accuracy improved significantly, when we added the best found combination of exogenous variables to the model. We found these to be weekend, wind, fossile, biomass, waste and exchange, as seen in the section "Forecasting".

The model is tested to outperform these models on multiple different sets of data, from the same time series data of the electricity spot prices in DK1, using validation data, crossvalidation and test data, to test for overfitting of the model.

From the time series crossvalidation, we also saw that the model, in general, had a higher accuracy than the auto.arima()-function, for generating automatic hyperparameters for the ARIMA model, based on a given time series.
