...
```{r}
prices %>%
  filter(price_area == "DK1", hour_utc >= "2020-01", hour_utc < "2020-02") %>%
  group_by(Hour) %>%
...
```{r}
plot(prices_trend$spot_price_dkk, type="line")
```

```{r}
plot(prod$biomass, type="line")
```

```{r}
plot(prod$fossil_gas, type="line")
```

```{r}
plot(prod$fossil_hard_coal, type="line")
```

```{r}
plot(prod$fossil_oil, type="line")
```

```{r}
plot(prod$hydro_power, type="line")
```

```{r}
plot(prod$solar_power, type="line")
```

```{r}
plot(prod$onshore_wind_power, type="line")
```

```{r}
plot(prod$offshore_wind_power, type="line")
```

```{r}
plot(con$flex_settled_consumption, type="line")
```

```{r}
plot(con$hourly_settled_consumption, type="line")
```
